...
internal static partial class WordConversions
{
   ...
   internal static Paragraph GetParagraph(OpenXmlElement section, FlowDocument fdoc, MainDocumentPart mainDocPart)
   {
      ...
      foreach (OpenXmlElement psection in section.Elements())
      {
         try
         {
            ...
            {
               case "pPr":
                  GetParagraphProperties(psection, ref para);
                  break;

               case "br": 
                  para.Inlines.Add(new EditableLineBreak()); 
                  break;
                  
               case "smartTag": 
                  AddDeepRuns(psection, ref para, mainDocPart); 
                  break;
                  
               case "hyperlink": 
                  para.Inlines.Add(GetEditableHyperlink(psection, ref para, mainDocPart)); 
                  break;
               ...
               case "del": 
                  para.Inlines.Add(GetDeletedRun(psection, ref para, mainDocPart)); 
                  break;

               case "ins": 
                  para.Inlines.Add(GetInsertedRun(psection, ref para, mainDocPart)); 
                  break;
   ...
   private static void GetParagraphProperties(OpenXmlElement propsElem, ref Paragraph para)
   {
      foreach (OpenXmlElement pps in propsElem.Elements())
      {
